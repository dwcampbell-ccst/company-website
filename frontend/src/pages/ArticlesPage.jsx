...
import { articlesHero } from "../content/siteContent";
  ...
  return (
    <section className="max-w-6xl mx-auto px-4 py-10 md:py-14 space-y-8">
      <div className="glass-panel p-6 md:p-8 space-y-3">
        ...
        <h1 className="text-3xl md:text-4xl font-bold text-[#0f1a0f]">{articlesHero.title}</h1>
        <p className="text-gray-800 text-lg max-w-3xl">{articlesHero.subtitle}</p>
        <div className="flex flex-wrap gap-3 pt-1">
          ...
          >
            {articlesHero.secondaryCta.label}
          </Link>
          <span className="text-sm text-gray-700">{articlesHero.optInText}</span>
        </div>
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-[2fr_1fr]">
        <div className="space-y-6">
          {loading ? (
            ...
            <p className="text-sm text-gray-700">No published posts yet. Create one in the admin area.</p>
          ) : (
            <div className="space-y-6">
              {posts.map((post) => (
                <article
